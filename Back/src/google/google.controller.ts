import { Controller, Get, Req, Res, UseGuards, OnModuleInit } from '@nestjs/common';
...
export class GoogleController implements OnModuleInit{
  ...
  async onModuleInit() {
    ...
    service.logo_url = 'https://www.google.com/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png';
    try {
      await service.save();
    } catch (error) {
  ...
  @ApiOkResponse({ description: 'Endpoint to redirect to google authentification' })
  @Get('google')
  ...
  async googleAuthRedirect(@Req() req: any, @Res() res: Response) {
    ...
    res.redirect(`http://localhost:8081/auth/succes?token=${userToken.access_token}`);
    return;
  }
}
