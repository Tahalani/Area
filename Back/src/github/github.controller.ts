...
export class GitHubController implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    service.logo_url = 'https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png';
    try {
      await service.save();
    } catch (error) {
  ...
  @ApiOkResponse({ description: 'Endpoint to redirect to github authentification' })
  @ApiBearerAuth()
  @UseGuards(AuthGuard)
  @Get('GitHub')
  async GitHubAuth(@Req() req: any, @Res() res: Response) {
    res.redirect(`https://github.com/login/oauth/authorize?client_id=${process.env.GITHUB_CLIENT_ID}&redirect_uri=${process.env.gitHubRedirectURL}&scope=user:email&state=${req.user.email}`);
  }
